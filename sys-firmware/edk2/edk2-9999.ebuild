...
src_compile(){
	...
	if use hello-world; then
		...
		echo "fs0:\HelloWorld.efi" > Build/MdeModule/RELEASE_"${TAGNAME}"/X64/startup.nsh || die "Failed to
		create startup.nsh"
	fi

	if use qemu; then
		build --arch "${ARCH}" --platform OvmfPkg/OvmfPkgX64.dsc --tagname "${TAGNAME}" \
			--buildtarget RELEASE || die "Failed to build uefirom"
 #               build --arch "${ARCH}" --platform OptionRomPkg/OptionRomPkg.dsc --tagname "${TAGNAME}" \
 #                       --buildtarget RELEASE || die "Failed to build optionrom"
	fi
...
src_install(){
	...
	if use qemu; then
		...
		newins Build/OvmfX64/RELEASE_"${TAGNAME}"/FV/OVMF.fd uefibios.bin
#		newins Build/OptionRomPkg/RELEASE_"${TAGNAME}"/X64/CirrusLogic5430Dxe.rom vgabios-cirrus.bin
		dosym "../${PN}/qemu/uefibios.bin" /usr/share/qemu/uefibios.bin
	fi

	if use shell; then
		insinto "/usr/share/${PN}/shell"
		newins Build/Shell/RELEASE_"${TAGNAME}"/X64/Shell.efi Shellx64.efi
	fi
}
